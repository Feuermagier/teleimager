...
class ImageClient:
    def __init__(self, host="192.168.123.164", request_port=60000):
        ...
        if (
            ...
            logger_mp.warning(
                "[Image Client] NOTICE! Head camera is not enabled on both ZMQ and WebRTC."
            )
...
def main():
    ...
    parser.add_argument(
        "--host", type=str, default="192.168.123.164", help="IP address of image server"
    )
    ...
    while running:
        if cam_config["head_camera"]["enable_zmq"]:
            ...
            if head_img is not None:
                ...
                logger_mp.debug(
                    f"Head Camera Shape: {cam_config['head_camera']['image_shape']}"
                )
                logger_mp.debug(
                    f"Head Camera Binocular: {cam_config['head_camera']['binocular']}"
                )
        ...
        if cam_config["left_wrist_camera"]["enable_zmq"]:
            ...
            if left_wrist_img is not None:
                ...
                logger_mp.debug(
                    f"Left Wrist Camera Shape: {cam_config['left_wrist_camera']['image_shape']}"
                )
        ...
        if cam_config["right_wrist_camera"]["enable_zmq"]:
            ...
            if right_wrist_img is not None:
                ...
                logger_mp.debug(
                    f"Right Wrist Camera Shape: {cam_config['right_wrist_camera']['image_shape']}"
                )
